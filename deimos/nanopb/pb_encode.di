/* pb_encode.di: Functions to encode protocol buffers. Depends on pb_encode.c.
...
module deimos.nanopb.pb_encode;
public import deimos.nanopb.pb;
extern(C):
...
{
    bool function(pb_ostream_t *stream, const(ubyte) *buf, size_t count) callback;
    void *state; /* Free field for use by callback implementation */
    size_t max_size; /* Limit number of output bytes written (or use SIZE_MAX). */
    size_t bytes_written;
}

pb_ostream_t pb_ostream_from_buffer(ubyte *buf, size_t bufsize);
bool pb_write(pb_ostream_t *stream, const(ubyte) *buf, size_t count);
...
bool pb_encode_varint(pb_ostream_t *stream, ulong value);

/* Encode an integer in the zig-zagged svarint format.
 * This works for sint32 and sint64. */
bool pb_encode_svarint(pb_ostream_t *stream, long value);

/* Encode a string or bytes type field. For strings, pass strlen(s) as size. */
bool pb_encode_string(pb_ostream_t *stream, const(ubyte) *buffer, size_t size);
